...
public class HomesMenu {
    ...
    private HomesGui homesMenu;
    private Essentials essentials = Main.getInstance().getEss();

    public HomesMenu(Config config) {
        this.config = config;
        this.homesMenu = config.getHomesGui();
    }

    public PaginatedGui get(Player player) {
        var user = essentials.getUser(player);
        var gui = Gui.paginated().title(Component.text(homesMenu.title()))
                .rows(homesMenu.rows()).pageSize(homesMenu.maxPageItems()).disableAllInteractions().create();
        if (homesMenu.menuItems() != null && !homesMenu.menuItems().isEmpty()) {
            homesMenu.menuItems().forEach(menuItem -> {
    ...
    private void setHomes(PaginatedGui gui, User user, Player player) {
        var homeMenuItem = homesMenu.homeItem();
